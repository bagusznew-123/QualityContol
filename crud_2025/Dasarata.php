...
if (isset($_GET['hal']) && $_GET['hal'] == "edit" && isset($_GET['id'])) {
    $id_edit = $_GET['id'];
    $query_edit = mysqli_query($koneksi, "SELECT * FROM qc_dasarata WHERE id_barang='$id_edit'");
...
if (isset($_POST['bsimpan'])) {
    ...
    if (isset($_GET['hal']) && $_GET['hal'] == "edit" && isset($_GET['id'])) {
        ...
        if ($ubah) {
            ...
            if ($sheetResult) {
                echo "<script>alert('✏️ Data berhasil diubah dan disinkronisasi ke spreadsheet!');document.location='Dasarata.php';</script>";
            } else {
                echo "<script>alert('✏️ Data berhasil diubah di database lokal, tetapi gagal disinkronisasi ke spreadsheet.');document.location='Dasarata.php';</script>";
            }
        } else {
        ...
        if ($simpan) {
            ...
            if ($sheetResult) {
                echo "<script>alert('✅ Data berhasil disimpan dan disinkronisasi ke spreadsheet!');document.location='Dasarata.php';</script>";
            } else {
                echo "<script>alert('✅ Data berhasil disimpan di database lokal, tetapi gagal disinkronisasi ke spreadsheet.');document.location='Dasarata.php';</script>";
            }
        } else {
...
if (isset($_GET['hal']) && $_GET['hal'] == "hapus") {
    ...
    $query_hapus_data = mysqli_query($koneksi, "SELECT * FROM qc_dasarata WHERE id_barang='$id_hapus'");
    $data_hapus = mysqli_fetch_array($query_hapus_data);
    
    $hapus = mysqli_query($koneksi, "DELETE FROM qc_dasarata WHERE id_barang='$id_hapus' ");
    if ($hapus) {
        // Data untuk dikirim ke Google Sheet
        $sheetData = array(
            'action' => 'delete',
        ...
        if ($sheetResult) {
            echo "<script>alert('🗑️ Data berhasil dihapus dan disinkronisasi ke spreadsheet!');document.location='Dasarata.php';</script>";
        } else {
            echo "<script>alert('🗑️ Data berhasil dihapus dari database lokal, tetapi gagal disinkronisasi ke spreadsheet.');document.location='Dasarata.php';</script>";
        }
    }
}

// Query data
$tampil_data = mysqli_query($koneksi, "SELECT * FROM qc_dasarata ORDER BY id_barang DESC");
...
<head>
  ...
  <title>Warehouse Modem - Dasarata Dashboard</title>
...
<div class="main-container">
  <div class="header">
    <h3><i class="fas fa-tachometer-alt me-2"></i>QC Dasarata Dashboard</h3>
  </div>
                ...
                <tr>
                  ...
                  <td class="text-center">
                    <div class="action-buttons">
                      <a href="Dasarata.php?hal=edit&id=<?= $data['id_barang'] ?>" class="btn btn-warning btn-sm">
